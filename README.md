...
## 抽出項目
書類の種類(受付連絡/支払完了はがき)は**画像から自動判定**。受付・支払を混ぜてアップロードできる。

- **受付書類:** 車両番号 / 発生日 / 補償の種類 / 運転者名
- **支払書類(完了はがき):** 車両番号 / 発生日 / 運転者名(はがきの「被保険者名」を読み取り) / 支払完了(はい/いいえ)

送信時に「通知種類」列へ **受付連絡** / **支払完了通知**(両方届いた行は「受付連絡・支払完了通知」)が自動記録される。
...
### 動作確認(スモーク)
1. パスワードでログインできる
2. 画像をドラッグ&ドロップ →「読み取り」で結果が表で出る(「書類種別」が自動判定される)
3. 表のセルを編集できる
4. 「スプレッドシートに送信」で対象シートに行が追加/更新され、成功メッセージが出る
...
## 使い方の流れ
1. ログイン(共有パスワード)
2. 画像をドラッグ&ドロップ(受付・支払を混ぜて複数可)
3. 「読み取り」→ 編集可能な表で結果を確認・修正(書類種別の誤判定もここで直す)
4. 「スプレッドシートに送信」→ 車両番号＋発生日で受付・支払を突合して追記
...
## ファイル構成
- `app.py` … Streamlit UI(ログイン・D&D・自動判定結果の確認表・送信)
- `schema.py` … 列・項目定義
- `matcher.py` … 突合と行マージの純関数
- `extract.py` … Gemini呼び出し(画像→書類種別判定＋項目抽出)
